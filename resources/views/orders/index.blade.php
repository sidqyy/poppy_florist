...
<div class="mb-6">
    ...
    <div class="bg-white p-4 rounded-xl shadow-md border-2 border-gray-200">
        <form action="{{ route('orders.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            <!-- Retain global search query if exists -->
            @if(request('q'))
            ...
            <div class="flex gap-2">
                <button type="submit" class="px-4 py-2 bg-florist-500 text-white font-bold rounded-lg hover:bg-florist-600 transition-colors text-sm shadow-sm">Terapkan Filter</button>
                @if(request('q') || request('date') || request('status') || request('payment_status'))
                    <a href="{{ route('orders.index') }}" class="px-4 py-2 bg-gray-100 text-gray-600 font-bold rounded-lg hover:bg-gray-200 transition-colors text-sm">Reset</a>
                @endif
            </div>
        </form>
    </div>
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="text-gray-600 divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="window.location='{{ route('orders.show', $order->id) }}'">
                    ...
                    <td class="py-4 px-4 text-xs">{{ $order->created_at->format('d/m/Y H:i') }}</td>
                    <td class="py-4 px-4 font-bold text-florist-600">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                    <td class="py-4 px-4">
                        @if($order->payment_status == 'paid')
                            <span class="px-2 py-1 bg-green-50 text-green-600 rounded text-xs font-bold">LUNAS</span>
                        @elseif($order->payment_status == 'dp')
                            <span class="px-2 py-1 bg-blue-50 text-blue-600 rounded text-xs font-bold">DP</span>
                        @else
                            <span class="px-2 py-1 bg-red-50 text-red-600 rounded text-xs font-bold">BELUM BAYAR</span>
                        @endif
                    </td>
                    <td class="py-4 px-4">
                    ...
                    <td class="py-4 px-4 text-right">
                        <a href="{{ route('orders.show', $order->id) }}" class="inline-block px-3 py-1.5 bg-white border border-gray-200 text-florist-500 hover:bg-florist-50 rounded-lg shadow-sm font-medium text-xs transition-colors">
                            Lihat Detail
                        </a>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="7" class="py-12 text-center text-gray-400">
                        ...
                        @if(request('q') || request('date') || request('status') || request('payment_status'))
                            ...
                            <a href="{{ route('orders.index') }}" class="text-florist-500 hover:underline mt-2 inline-block text-sm">Hapus Filter</a>
                        @else
