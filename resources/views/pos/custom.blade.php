        ...
        <div class="max-w-6xl mx-auto">
            ...
            <form action="{{ route('pos.cart.add-custom') }}" method="POST" id="customForm" class="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div class="flex-1 bg-white rounded-2xl p-6 shadow-md border-2 border-gray-200 w-full">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 font-bold mb-2 text-sm">Beri Nama Rakitan Ini <span class="text-red-500">*</span></label>
                    ...
                    <div class="mb-6">
                        <label class="block text-gray-700 font-bold mb-2 text-sm">Catatan Perangkaian (Opsional)</label>
                ...
                <!-- Tambahan Produk Custom -->
                <div id="extra-section" class="hidden mb-6">
                    <h3 class="text-lg font-bold text-gray-800 mb-3 border-b pb-2">Daftar Tambahan Produk Custom</h3>
                    <div id="extra-items-container" class="flex flex-col gap-3 mb-4">
                        <!-- Baris dynamic akan muncul di sini -->
                    </div>
                ...
                <h3 class="text-lg font-bold text-gray-800 mb-3 border-b pb-2">{{ $step++ }}. Pilih {{ $typeLabels[$type] ?? ucwords(str_replace('_', ' ', $type)) }}</h3>
                <div class="flex flex-col gap-3 mb-6">
                    @foreach($items as $item)
                    <div class="border rounded-xl p-2.5 flex items-center gap-3 {{ $item->stock <= 0 && $item->type !== 'service' ? 'opacity-50 bg-gray-50' : 'hover:border-purple-300 transition-colors' }}">
                        @if($item->image)
                        <img src="{{ Storage::url($item->image) }}" class="w-12 h-12 object-cover rounded-lg shadow-sm shrink-0">
                        @else
                        <div class="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 shadow-sm text-xl shrink-0"><i class="fa-solid {{ $typeIcons[$type] ?? 'fa-box-open' }}"></i></div>
                        @endif
                        
                        <div class="flex-1 text-left min-w-0">
                            <h4 class="font-bold text-gray-800 text-sm leading-tight truncate">{{ $item->name }}</h4>
                            <p class="text-purple-600 font-bold text-sm">Rp {{ number_format($item->price, 0, ',', '.') }}<span class="text-xs text-gray-400">/{{ $item->unit }}</span></p>
                        </div>
                        
                        @if($item->stock > 0 || $item->type === 'service')
                        <div class="flex items-center gap-1 bg-gray-100 rounded-lg p-1 w-24 justify-between shrink-0">
                            <button type="button" onclick="adjustQty('mat_{{ $item->id }}', -1)" class="w-7 h-7 rounded bg-white shadow-sm flex items-center justify-center text-gray-600 font-bold touch-btn"><i class="fa-solid fa-minus text-xs"></i></button>
                            <input type="number" name="materials[{{ $item->id }}]" id="mat_{{ $item->id }}" data-name="{{ $item->name }}" data-price="{{ $item->price }}" value="0" min="0" max="{{ $item->type === 'service' ? 999999 : $item->stock }}" class="w-6 bg-transparent text-center font-bold outline-none text-sm p-0 border-none focus:ring-0 custom-mat-input" readonly>
                            <button type="button" onclick="adjustQty('mat_{{ $item->id }}', 1, {{ $item->type === 'service' ? 999999 : $item->stock }})" class="w-7 h-7 rounded bg-white shadow-sm flex items-center justify-center text-purple-600 font-bold touch-btn"><i class="fa-solid fa-plus text-xs"></i></button>
                        </div>
                        @else
                ...
                <div class="w-full md:w-80 shrink-0 bg-white rounded-2xl p-5 shadow-md border-2 border-gray-200 sticky top-6">
                    <h3 class="font-bold text-gray-800 mb-4 text-lg"><i class="fa-solid fa-clipboard-list text-purple-500 mr-2"></i> Ringkasan Rakitan</h3>
                    
                    <div id="custom-summary-box" class="mb-4 hidden">
                        <div id="summary-items" class="text-sm text-gray-600 mb-3 flex flex-col gap-2">
...
<script>
    ...
    function updateSummary() {
        ...
        document.querySelectorAll('.custom-mat-input').forEach(input => {
        ...
        if (document.getElementById('custom_type_selector').value === 'produk') {
            document.querySelectorAll('#extra-items-container > div').forEach(row => {
                let nameInput = row.querySelector('.extra-name-input');
                let priceInput = row.querySelector('.extra-price-input');
                let qtyInput = row.querySelector('.extra-qty-input');
                
                let name = nameInput.value.trim() || 'Produk Tambahan';
    ...
    function setCustomType(type) {
        ...
        if (type === 'bunga') {
            ...
            // Clear extra items if they switch back to bunga
            ...
            // Add at least 1 empty item if it's empty
        ...
        const html = `
            <div id="${rowId}" class="border border-purple-100 bg-purple-50 rounded-xl p-3 flex flex-col md:flex-row items-center gap-3 relative">
                ...
                <div class="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-1 w-24 justify-between shrink-0">
                    <button type="button" onclick="adjustExtraQty('${rowId}_qty', -1)" class="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-gray-600 font-bold touch-btn"><i class="fa-solid fa-minus text-xs"></i></button>
                    ...
                    <button type="button" onclick="adjustExtraQty('${rowId}_qty', 1)" class="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-gray-600 font-bold touch-btn"><i class="fa-solid fa-plus text-xs"></i></button>
                </div>
            </div>
        `;
