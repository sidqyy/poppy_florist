...
class DashboardController extends Controller
{
    public function admin()
    {
        ...
        $todayRevenue = Payment::whereDate('verified_at', $today)->where('status', 'verified')->sum('amount');
        ...
        $topProducts = OrderItem::select('product_name', DB::raw('count(*) as total_sales'), DB::raw('sum(subtotal) as total_revenue'))
        ...
        // Active rentals
        $activeRentals = OrderItem::where('is_rented', true)
            ->whereHas('order', function ($q) {
                $q->whereNotIn('status', ['cancelled']);
            })->count();

        // Rentals due today
        $rentalsDueToday = OrderItem::where('is_rented', true)
            ->whereHas('order', function ($q) {
                $q->whereNotIn('status', ['cancelled']);
            })
            ->get()
        ...
        $materialsUsedToday = OrderItemComponent::select('material_name', DB::raw('sum(qty) as total_qty'))
